<?php
   ...
   if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        {
            $username_err="admin name cannot be empty";
        }
        else
        {
            $sql="SELECT id from admins WHERE admin_name=?";
            ...
            if($stmt)
            {
                ...
                if(mysqli_stmt_execute($stmt))
                {
                    ...
                    {
                        $admin_name_err="This admin_name is already taken!";
                    }
                    else{
...
if(empty($admin_name_err)&&empty($password_err)&&empty($confirm_password_err))
{
    $sql = "INSERT INTO admins (admin_name,dob,gender,mobile_no,email,password) values(?,?,?,?,?,?)";
    $stmt=mysqli_prepare($conn,$sql);
    if($stmt)
    {
        mysqli_stmt_bind_param($stmt,'ssssss',$param_admin_name,$param_dob,$param_gender,$param_mobile_no,$param_email,$param_password);
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  <div class="container-fluid">
    ...
    <div class="container">
      ...
      <div class="col-md-12">
        <label for="inputusername" class="form-label">Admin_name</label>
        <input type="text" name="admin_name" class="form-control" id="inputusername" placeholder="Enter the Admin name">
      </div>
      <div class="col-md-12">
        <label for="inputage" class="form-label">Date of Birth</label>
